...
def selective_state_update(state, x, dt, A, B, C, D=None, z=None, dt_bias=None, dt_softplus=False):
    ...
    # Use CUDA device only if available, otherwise fall back to reference
    if x.device.type == "cuda":
        ...
        # Fall back to reference implementation for non-CUDA devices (e.g., MPS)
        out = selective_state_update_ref(state, x, dt, A, B, C, D, z, dt_bias, dt_softplus)
        if not has_heads:
            out = out.squeeze(1)
        return out
    if not has_heads:
